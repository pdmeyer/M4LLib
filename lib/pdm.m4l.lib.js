...
class M4LLib {
        ...
        try {
            if(!this.validateId(deviceId)) {
                throw new M4LLibErrors.M4LLibError(
                    ...
                    'VALIDATION_ERROR',
                    { received: deviceId, suggestion: 'Use M4LLib.conformId() to convert formats' },
                    methodName
                );
            }
     ...
     */
    static dumpNotesToNextEmptyClipForTrack(trackId, notes) {
        const methodName = 'M4LLib.dumpNotesToNextEmptyClipForTrack';
        
        try {
            if(!this.validateId(trackId)) {
                throw new M4LLibErrors.M4LLibError(
                    'Track ID must be a valid number',
                    'VALIDATION_ERROR',
                    { received: trackId, suggestion: 'Use M4LLib.conformId() to convert formats' },
                    methodName
                );
            }
            ...
            if (!nextEmptyClipSlotId) {
                throw new M4LLibErrors.M4LLibError(
                    'No empty clip slot found for track',
            ...
            const clipLength = this.calculateClipLengthFromNotes(notes);
    ...
    static addNotesToClip(clipId, notes) {
        ...
        try {
            // Validate inputs
            if(!this.validateId(clipId)) {
                throw new M4LLibErrors.M4LLibError(
                    'Clip ID must be a valid number',
                    'VALIDATION_ERROR',
                    { received: clipId, suggestion: 'Use M4LLib.conformId() to convert formats' },
                    methodName
                );
            }
    ...
    static createNewEmptyMidiClip(length, clipSlotId) {
        ...
        try {
            clipSlot = new LiveAPI(clipSlotId);
            const trackPath = this.getTrackPathFromPath(clipSlot.path);
            if(trackPath == null) {
                throw new M4LLibErrors.M4LLibError(
                    'Failed to get track path from clip slot path',
                    'LIVE_API_ERROR',
                    { clipSlotPath: clipSlot.path },
                    methodName
                );
            }
     ...
     * @throws {M4LLibError} If track cannot have MIDI clips, track access fails, or the operation fails
     * 
     * @example
    ...
    static getNextEmptyClipSlotIdForTrack(trackId, start_index = 0) {
        ...
        return emptyClipSlot ? emptyClipSlot.id : null;
    };
     ...
     */
    static withLiveAPI(pathOrId, operation, context = null, args = []) {
        const methodName = 'M4LLib.withLiveAPI';
        let liveApi = null;
        ...
        try {
            ...
            // Convert numeric IDs to "id N" format for LiveAPI constructor
            const liveApiPath = typeof pathOrId === 'number' ? this.prefixId(pathOrId) : pathOrId;
            
            // Create the LiveAPI object
            liveApi = new LiveAPI(liveApiPath);
...
// Temporarily commented out to test JSDoc generation
