...
class M4LLib {
    ...
    //properties of all objects that can be queried with basic JS syntax (object.property)
    static LiveApiProperties = ['children', 'id', 'info', 'path', 'mode', 'patcher', 'property', 'proptype', 'type', 'unquotedpath'];

    //properties that are only accessible under certain conditions
    static ConditionalProperties = {
        'value_items': {
            'DeviceParameter': {
                'condition': (object) => {return object.get('is_quantized') == 1}
            }
        }
    };
     ...
     * @returns {Array} An array containing the results of calling the callback function for each ID
     * 
     * @example
    ...
    static iterateIds(id_array, fn) {
        id_array = this.conformIdArray(id_array);
        const response = [];
     ...
     * @returns {string} The index of the item as a string
     ...
     * 
     * @static
     * @param {Object} object - The LiveAPI object to get the property from
     * @param {string} property - The name of the property to get
     * @returns {*} The property value, or null if the property is not accessible
     */
    static getObjectProperty(object, property) {
        const methodName = 'M4LLib.getObjectProperty';
        
        if (M4LLib.LiveApiProperties.includes(property)) {
            return object[property];
        } else {
            // Check if this property has conditional access rules
            if (M4LLib.ConditionalProperties[property]) {
                const objectType = object.type;
                const conditionalRule = M4LLib.ConditionalProperties[property][objectType];
                
                if (conditionalRule) {
                    if (typeof conditionalRule.condition !== 'function') {
                        throw new M4LLibErrors.M4LLibError(
                            'Conditional rule is not a function',
                            'VALIDATION_ERROR',
                            { conditionalRule: conditionalRule.condition },
                            methodName
                        );
                    } else {
                        if (!conditionalRule.condition(object)) {
                            return null;
                        }
                    }
                }
     ...
     * 
     * @static
     * @param {string|number} id - The ID of the object to get information for
     * @returns {Object} An object containing the parsed info string with properties and their types
     */
    static getObjectInfo(id) {
        const methodName = 'M4LLib.getObjectInfo';
        const object = new LiveAPI(this.prefixId(id));
        const info = object.info.split('\n');
        const infoObj = {};
        info.forEach((line) => {
            ...
            if (spaceIndex > 0) {
                ...
                // Special handling for property entries (name type format)
                if (key === 'property') {
                    const lastSpaceIndex = value.lastIndexOf(' ');
                    const propertyName = lastSpaceIndex > 0 ? value.substring(0, lastSpaceIndex) : value;
                    const propertyType = lastSpaceIndex > 0 ? value.substring(lastSpaceIndex + 1) : 'unknown';
                    
                    if (!infoObj[key]) {
                        infoObj[key] = {};
                    }
                    infoObj[key][propertyName] = propertyType;
                } else {
                    // If key already exists, convert to array or add to existing array
                    if (infoObj[key]) {
                        if (Array.isArray(infoObj[key])) {
                            infoObj[key].push(value);
                        } else {
                            infoObj[key] = [infoObj[key], value];
                        }
                    } else {
                        infoObj[key] = value;
                    }
     ...
     * 
     * @static
     * @param {string|number} id - The ID of the parent object to get children from
     * @param {string} childClass - The class name of the children to retrieve (e.g., 'parameters', 'tracks')
     * @returns {Array<number>|null} An array of child IDs, or null if the operation fails
     */
    static getChildren(id, childClass) {
        const methodName = 'M4LLib.getChildren';
        try {
            const parent = new LiveAPI(this.prefixId(id));
            const children = this.conformIdArray(parent.get(childClass));
            parent.freepeer();
            return children;
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
     ...
     * @param {string|number} id - The ID of the parent object to get children from
     * @param {string} childClass - The class name of the children to retrieve (e.g., 'parameters', 'tracks')
     * @param {Array<string>} [childProperties=null] - Array of property names to retrieve for each child. If null, uses all valid properties
     * @returns {Object|null} An object where keys are child IDs and values are objects containing the requested properties, or null if the operation fails
     * @throws {M4LLibError} If the parent ID is invalid, child class is not valid for the parent, or the operation fails
     * 
     * @example
     ...
     */
    static getPropertiesForChildren(id, childClass, childProperties = null, key = 'id') {
        const methodName = 'M4LLib.getPropertiesForChildren';
        try {
            const children = this.getChildren(id, childClass);
            post('children', children, '\n');
            return this.getPropertiesForObjects(children, childProperties, key);
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
     ...
     * 
     * @static
     * @param {Array<number>} idArray - Array of object IDs to get properties for
     * @param {Array<string>} [properties=null] - Array of property names to retrieve. If null, uses all valid properties
     * @param {string} [key='id'] - The key to use for the object IDs. 
     * @returns {Object|null} An object where keys are object IDs and values are objects containing the requested properties, or null if the operation fails
     * 
     * Note: If you specify a key, make sure it is unique for each object in the idArray. Otherwise, the properties will be overwritten.
     */
    static getPropertiesForObjects(idArray, properties = null, key = 'id') {
        const methodName = 'M4LLib.getPropertiesForObjects';
        try {
            idArray = this.conformIdArray(idArray);
            
            if(properties !== null && !Array.isArray(properties)) {
                throw new M4LLibErrors.M4LLibError(
                    'Properties parameter must be an array',
                    'VALIDATION_ERROR',
                    { received: typeof properties, expected: 'array' },
                    methodName
                );
            }

            const validProperties = [...Object.keys(this.getObjectInfo(idArray[0]).property), 'id'] || [];

            if (properties !== null && !properties.every(prop => validProperties.includes(prop))) {
                const invalidProps = properties.filter(prop => !validProperties.includes(prop));
                throw new M4LLibErrors.M4LLibError(
                    'Invalid property names: ' + invalidProps.join(', '),
                    'VALIDATION_ERROR',
                    { received: invalidProps, expected: 'valid properties', validProperties: validProperties },
                    methodName
                );
            }

            if(![...properties, 'id'].includes(key)) {
                throw new M4LLibErrors.M4LLibError(
                    'Key parameter must be one of: ' + [...properties, 'id'].join(', '),
                    'VALIDATION_ERROR',
                    { received: key, expected: 'valid key' },
                    methodName
                );
            }
            ...
            const infoArray = this.iterateIds(idArray, (id) => {
                const object = new LiveAPI(this.prefixId(id));
                const data = properties.reduce((acc, property) => {
                    acc[property] = this.getObjectProperty(object, property);
                    return acc;
                }, {});
                object.freepeer();
            ...
            infoArray.forEach((info) => {
                const keyValue = key == 'id' ? info.id : info.data[key];
                infoObj[keyValue] = info.data;
            });
            return infoObj;
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
     ...
     * @returns {Object|null} An object where keys are parameter IDs and values are objects containing {name: string, id: number}, or null if the operation fails
     */
    static getDeviceParameterNames(deviceId) {
        post('getDeviceParameterNames', deviceId, '\n');
        const methodName = 'M4LLib.getDeviceParameterNames';
        try {
            post('callinggetPropertiesForChildren', deviceId, 'parameters', ['id', 'name'], 'name', '\n');
            return this.getPropertiesForChildren(deviceId, 'parameters', ['id', 'name'], 'name');
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
     ...
     * @static
     * @param {string|number} deviceId - The ID of the device to get the parameters for
     * @returns {Array<number>|null} An array of parameter IDs, or null if the operation fails
     */
    static getDeviceParameters(deviceId) {
        const methodName = 'M4LLib.getDeviceParameters';
        try {
            return this.getChildren(deviceId, 'parameters');
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
     ...
     * @static
     * @param {string|number} deviceId - The ID of the device to get the parameters for
     * @param {Array<string>} [properties=[]] - Array of property names to retrieve for each parameter
     * @returns {Object|null} An object where keys are parameter IDs and values are objects containing the requested properties, or null if the operation fails
     */
    static getDeviceParameterInfo(deviceId, properties = []) {
        const methodName = 'M4LLib.getDeviceParameterInfo';
        try {
            return this.getPropertiesForChildren(deviceId, 'parameters', properties);
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
     ...
     * 
     * @static
     * @param {string|number} id - The ID of the object to get the path for
     * @returns {string|null} The path of the object, or null if the operation fails
     */
     ...
     * 
     * @static
     * @param {Array} id_array - The array of IDs to convert (can be in "id N" format or just numbers)
     * @returns {Array<number>} An array containing only the numeric IDs
     */
    static conformIdArray(id_array) {
        const idsOnly = [];
        for (let i = 0; i < id_array.length; i++) {
            if (typeof id_array[i] == 'number') {
                idsOnly.push(id_array[i]);
            }
        }
        return idsOnly;
    }
    ...
    // ========================================================================
    // GENERAL MAX UTILITIES
    // ========================================================================

    /**
     * Javascript object to Max dictionary
     * 
     * @static
     * @param {Object} obj - The Javascript object to convert to a Max dictionary
     * @param {Dict} dict - The Max dictionary to convert the object to (optional)
     * @returns {Dict} The Max dictionary containing the converted object
     * 
     *  Note: Dictionaries are not automatically freed by the JS garbage collector, 
     *  so it is your responsibility to free them using Dict.freepeer() when you are done using them.
     */
    static objToDict(obj, dict = null) {
        const methodName = 'M4LLib.objToDict';
        if (dict == null) {
            dict = new Dict();
        } else if (!(dict instanceof Dict)) {
            throw new M4LLibErrors.M4LLibError(
                'Dict parameter must be a Max dictionary',
                'VALIDATION_ERROR',
                { received: typeof dict, expected: 'Dict' },
                methodName
            );
        }
    ...
    /**
     * Max dictionary to Javascript object
     * 
     * @static
     * @param {Dict} dict - The Max dictionary to convert to a Javascript object
     * @returns {Object} The Javascript object containing the converted dictionary
     * 
     *  Note: Dictionaries are not automatically freed by the JS garbage collector, 
     *  so it is your responsibility to free them using Dict.freepeer() when you are done using them.
     */
    static dictToObj(dict) {
        const methodName = 'M4LLib.dictToObj';
