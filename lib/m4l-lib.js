...
class M4LLib {
     ...
     */
    static getPropertiesForChildren(id, childClass, childProperties = null) {
        const methodName = 'M4LLib.getPropertiesForChildren';
        try {
            const children = this.getChildren(id, childClass);
            return this.getPropertiesForObjects(children, childProperties);
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
     ...
     */
    static getPropertiesForObjects(idArray, properties = null) {
        const methodName = 'M4LLib.getPropertiesForObjects';
        try {
            idArray = this.conformIdArray(idArray);
            const validProperties = Object.keys(this.getObjectInfo(idArray[0]).property) || [];
            
            if (properties === null) {
                //childProperties = M4LLib.LiveApiProperties.concat(validProperties);
    ...
    static getDeviceParameterNames(deviceId) {
        ...
        try {
            return this.getPropertiesForChildren(deviceId, 'parameters', ['id', 'name']);
        } catch (error) {
            M4LLibErrors.ErrorHandler.handle(error, methodName, false);
            return null;
        }
