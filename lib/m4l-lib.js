...
class M4LLib {
    ...
    static getDeviceParameterNames(deviceId) {
        ...
        try {
            ...
            device.freepeer();
            const parameterInfoArray = this.iterateIds(parameterIds, (id) => {
            ...
            parameterInfoArray.forEach((info) => {
                parameterInfoObj[info.name] = info;
            });
     ...
     */
        static getDeviceParameterInfo(deviceId, properties = null) {
            const methodName = 'M4LLib.getDeviceParameterNames';
            const liveApiProperties = ['children', 'id', 'info', 'path', 'mode', 'patcher', 'property', 'proptype', 'type', 'unquotedpath'];
            if(properties === null) {
                properties = ['name', 'automation_state', 'default_value', 'is_enabled', 'is_quantized', 'max', 'min', 'original_name', 'state', 'value', 'display_value'];
            }
        try {
            this.validateId(deviceId, methodName);
            const device = new LiveAPI(this.prefixId(deviceId));
            ...
            const parameterInfoArray = this.iterateIds(parameterIds, (id) => {
                ...
                const data = properties.reduce((acc, property) => {
                    if(liveApiProperties.includes(property)) {
                        acc[property] = parameter[property];
                    } else {
                        acc[property] = parameter.get(property)[0];
                    }
                    return acc;
                }, {});
                parameter.freepeer();
